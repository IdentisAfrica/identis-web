...
export default function LivenessStep({ verificationId, onNext }: Props) {
  ...
  const names: Record<string,string> = { center: '🎯 Center face', closer: '🔍 Move closer', turn: '↔️ Turn head' }
  ...
  const submitResult = useCallback(async (score: number) => {
    ...
    } catch (e) { setError('Save failed'); setStep('failed') }
  ...
  const processFrame = useCallback(async () => {
    ...
    try {
      ...
      if (!hasFace) { setDebug('Searching...'); animRef.current = requestAnimationFrame(processFrame); return }
      ...
      if (step === 'checking' && stableFace.current) {
        ...
        let pass = ch === 'center' ? Math.abs(cx-0.5) < 0.2 && Math.abs(cy-0.5) < 0.2 && size > 0.12 : ch === 'closer' ? size > 0.25 : Math.abs(cx-0.5) > 0.08
        if (pass) {
          holdFrames.current++
          setInstruction(names[ch] + ' - Hold (' + holdFrames.current + '/5)')
          if (holdFrames.current >= 5) {
            completed.current.push(ch); challengeIdx.current++; holdFrames.current = 0
            ...
            setInstruction(names[challenges[challengeIdx.current]])
          }
        } else { holdFrames.current = Math.max(0, holdFrames.current - 0.5); setInstruction(names[ch]) }
      }
  ...
  const startCheck = useCallback(() => {
    ...
    setStep('checking'); setProgress(0); setError(''); setInstruction(names[challenges[0]])
  }, [])

  const retry = useCallback(() => {
    challengeIdx.current = 0; holdFrames.current = 0; completed.current = []
    setStep('ready'); setError(''); setProgress(0); setInstruction('Position face, tap Start')
    animRef.current = requestAnimationFrame(processFrame)
  }, [processFrame])
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setLoading(false); setStep('ready'); setInstruction('Position face, tap Start')
        animRef.current = requestAnimationFrame(processFrame)
  ...
  return (
    <div style={{ maxWidth: 500, margin: '0 auto', padding: 20 }}>
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3', background: '#000', borderRadius: 12, overflow: 'hidden', marginBottom: 20 }}>
        ...
        {step === 'submitting' && <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(30,58,95,0.9)', color: '#fff' }}><p>Saving...</p></div>}
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: 15 }}>
        ...
        {step === 'failed' && <button onClick={retry} style={{ padding: '14px 32px', fontSize: 16, fontWeight: 'bold', color: '#fff', background: '#dc2626', border: 'none', borderRadius: 8 }}>Retry</button>}
      </div>
    </div>
  )
}
